"""Comprehensive tests for SushiSwap DEX adapter."""
...
@pytest.fixture
def router_address():
    """Fixture providing SushiSwap router address."""
    return "0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506"


@pytest.fixture
def sushiswap(router_address):
    """Fixture providing SushiSwap instance."""
    return SushiSwap(router_address=router_address)


@pytest.fixture
def sushiswap_custom_name(router_address):
    """Fixture providing SushiSwap instance with custom name."""
    return SushiSwap(router_address=router_address, name="Custom SushiSwap")
...
def test_initialization(router_address):
    """Test SushiSwap initialization with valid parameters."""
    sushiswap = SushiSwap(router_address=router_address)

    assert sushiswap.name == "SushiSwap"
    assert sushiswap.router_address == Web3.to_checksum_address(router_address)
    assert sushiswap.router_abi is not None
    assert len(sushiswap.router_abi) == 2  # getAmountsOut and swapExactTokensForTokens
    assert sushiswap.contract is None  # Not initialized yet


def test_initialization_custom_name(sushiswap_custom_name):
    """Test SushiSwap initialization with custom name."""
    assert sushiswap_custom_name.name == "Custom SushiSwap"


def test_initialization_checksums_address():
    """Test that initialization converts address to checksum format."""
    lowercase_router = "0x1b02da8cb0d097eb8d57a175b88c7d8b47997506"
    sushiswap = SushiSwap(router_address=lowercase_router)

    assert sushiswap.router_address == "0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506"
    assert sushiswap.router_address != lowercase_router


def test_router_abi_structure(sushiswap):
    """Test that router ABI has correct structure."""
    abi = sushiswap.router_abi
...
@pytest.mark.asyncio
async def test_get_token_price_success(sushiswap, mock_web3, token_addresses):
    ...
    sushiswap.contract = mock_router

    price = await sushiswap.get_token_price(
...
@pytest.mark.asyncio
async def test_get_token_price_default_amount(sushiswap, mock_web3, token_addresses):
    ...
    price = await sushiswap.get_token_price(token_addresses["usdc"], mock_web3)
...
@pytest.mark.asyncio
async def test_get_token_price_error_handling(sushiswap, mock_web3, token_addresses):
...
async def test_get_token_price_initializes_contract(
    sushiswap, mock_web3, token_addresses
):
    """Test that get_token_price initializes contract if needed."""
    assert sushiswap.contract is None
    ...
    await sushiswap.get_token_price(token_addresses["usdc"], mock_web3)

    # Contract should be initialized
    assert sushiswap.contract is not None


def test_get_best_path_direct(sushiswap, token_addresses):
    """Test that _get_best_path returns direct path."""
    path = sushiswap._get_best_path(token_addresses["usdc"], token_addresses["weth"])
...
def test_get_best_path_checksums_addresses(sushiswap):
    ...
    path = sushiswap._get_best_path(lowercase_token1, lowercase_token2)
...
@pytest.mark.asyncio
@patch("src.dex.sushiswap.Web3.to_checksum_address")
async def test_execute_trade_success(
    mock_checksum, sushiswap, mock_web3, token_addresses
):
    ...
    success, tx_hash = await sushiswap.execute_trade(
...
async def test_execute_trade_reverted(
    ...
    success, result = await sushiswap.execute_trade(
        token_addresses["usdc"],
        token_addresses["weth"],
        Decimal("1000"),
        mock_web3,
...
async def test_execute_trade_exception(
    ...
    success, error_msg = await sushiswap.execute_trade(
        token_addresses["usdc"],
        token_addresses["weth"],
        Decimal("1000"),
        mock_web3,
...
async def test_execute_trade_initializes_contract(
    ...
    with patch("src.dex.sushiswap.Web3.to_checksum_address", side_effect=lambda x: x):
        await sushiswap.execute_trade(
            token_addresses["usdc"],
            token_addresses["weth"],
            Decimal("1000"),
            mock_web3,
...
async def test_get_liquidity_depth_returns_simplified_values(
    ...
    liquidity, impact = await sushiswap.get_liquidity_depth(
        token_addresses["usdc"],
        token_addresses["weth"],
        mock_web3,
...
def test_sushiswap_inherits_from_dex(sushiswap):
    """Test that SushiSwap properly inherits from DEX."""
    from src.dex.base import DEX

    assert isinstance(sushiswap, DEX)
    assert hasattr(sushiswap, "get_token_price")
    assert hasattr(sushiswap, "execute_trade")
    assert hasattr(sushiswap, "get_liquidity_depth")
    assert hasattr(sushiswap, "initialize_contract")


def test_sushiswap_attributes_are_accessible(sushiswap):
    """Test that all SushiSwap attributes are properly set and accessible."""
    assert hasattr(sushiswap, "name")
    assert hasattr(sushiswap, "router_address")
    assert hasattr(sushiswap, "router_abi")
    assert hasattr(sushiswap, "contract")

    assert sushiswap.name == "SushiSwap"
    assert sushiswap.router_address is not None
    assert sushiswap.router_abi is not None
    assert sushiswap.contract is None  # Not initialized yet
